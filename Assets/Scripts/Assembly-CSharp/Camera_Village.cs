...
public class Camera_Village : MonoBehaviour
{
	...
	{
		float num = wapram / 200f;
		if (!(num > 1f) && !(num < -1f))
		{
			float num2 = Mathf.Asin(num) * 2f * (180f / (float)Math.PI);
			if (target_angle - num2 > angle_max)
			{
				float num3 = Mathf.Abs(base.transform.eulerAngles.y - angle_max);
				num3 = ((!(num3 >= 1f)) ? 1f : num3);
				num2 *= 0.25f / num3;
			}
			else if (target_angle - num2 < angle_min)
			{
				float num4 = Mathf.Abs(base.transform.eulerAngles.y - angle_min);
	...
	private void UpdateMove()
	{
		...
		y2 = Mathf.LerpAngle(y2, target_angle, rotation_damping * 4f * Time.deltaTime);
