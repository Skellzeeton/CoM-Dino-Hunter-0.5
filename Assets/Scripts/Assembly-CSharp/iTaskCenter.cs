...
public class iTaskCenter
{
	...
	public bool Load()
	{
		...
		{
			if (!Utils.FileGetString("task.xml", ref content))
			{
				return false;
			}
		}
		...
		{
			TextAsset textAsset = (TextAsset)Resources.Load(PrefabManager.GetPath(3008), typeof(TextAsset));
			if (textAsset == null)
			{
				return false;
			}
