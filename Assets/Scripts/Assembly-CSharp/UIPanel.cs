...
public class UIPanel : MonoBehaviour
{
    ...
    {
        if (!w.enabled || !NGUITools.GetActive(w.gameObject) || w.color.a < 0.001f)
        {
            return false;
        }
        if (mClipping == UIDrawCall.Clipping.None)
    ...
    private UIDrawCall GetDrawCall(Material mat, bool createIfMissing)
    {
        ...
        if (createIfMissing)
        {
            ...
            UIDrawCall newCall = go.AddComponent<UIDrawCall>();
            newCall.material = mat;

            mDrawCalls.Add(newCall);
    ...
    private void UpdateWidgets()
    {
        ...
        for (int count = mChildren.Count; i < count; i++)
        {
            ...
            if (uINode.visibleFlag == 1 && widget != null && widget.UpdateGeometry(ref mWorldToLocal, uINode.changeFlag == 1, generateNormals) && !mChanged.Contains(widget.material))
            {
...
#if UNITY_EDITOR
            EditorUtility.SetDirty(drawCall.gameObject);
            SceneView.RepaintAll();
#endif
        }
        else
