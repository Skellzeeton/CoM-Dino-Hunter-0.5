	...
	protected bool MGStart(List<object> ltParam)
	{
		...
		if (taskInfo != null && taskInfo.nType == 3)
		{
			...
			if (pathWalkerManager != null)
			{
				...
				foreach (WaveMobInfo item in waveInfo.m_ltWaveMobInfo)
				{
					if (item.nStartPoint == 0)
					{
						list.Clear();
						break;
					}
				...
				if (sPManagerGround != null && tPManagerEnd != null)
				{
					Dictionary<int, CStartPoint> data = sPManagerGround.GetData();
					Dictionary<int, CStartPoint> data2 = tPManagerEnd.GetData();
					foreach (KeyValuePair<int, CStartPoint> item3 in data)
					{
						...
						foreach (CStartPoint value in data2.Values)
						{
							List<Vector3> navMeshPath = m_GameScene.GetNavMeshPath(item3.Value.v3Pos, value.v3Pos);
							if (navMeshPath.Count > 0)
							{
								pathWalkerManager.Go(waveInfo.nID, navMeshPath);
							}
						}
					}
				}
