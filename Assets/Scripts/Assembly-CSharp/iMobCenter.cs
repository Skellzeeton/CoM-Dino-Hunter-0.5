...
public class iMobCenter
{
	...
	public bool Load()
	{
		...
		{
			if (!Utils.FileGetString("mob.xml", ref content))
			{
				return false;
			}
		}
		...
		{
			TextAsset textAsset = (TextAsset)Resources.Load(PrefabManager.GetPath(3001), typeof(TextAsset));
			if (textAsset == null)
			{
				return false;
			}
