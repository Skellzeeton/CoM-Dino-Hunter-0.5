...
public class TUISliderEx : TUIControlImpl
{
	...
	private void UpdateDrag(Vector3 lastTouchPosition)
	{
		...
		if (thumbRect != null)
		{
			Set((direction != Direction.Horizontal) ? ((vector.y - (base.transform.localPosition.y - size.y * 0.5f + thumbRect.Size.y * 0.5f)) / (size.y - thumbRect.Size.y)) : ((vector.x - (base.transform.localPosition.x - size.x * 0.5f + thumbRect.Size.x * 0.5f)) / (size.x - thumbRect.Size.x)), thumbRect, true);
		}
		else
		{
			Set((direction != Direction.Horizontal) ? (vector.y / size.y + 0.5f) : (vector.x / size.x + 0.5f), true);
		}
	}
