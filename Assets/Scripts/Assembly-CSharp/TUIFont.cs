...
public class TUIFont : MonoBehaviour
{
	...
	[SerializeField]
	protected TUIBMFont mFont = new TUIBMFont();

	protected List<Color> mColors = new List<Color>();

	public bool test;

	public TUIBMFont bmFont
	{
		get
		{
			return mFont;
	...
	{
		lineWidth = ((alignment != Alignment.Left) ? ((lineWidth <= 0) ? Mathf.RoundToInt(CalculatePrintedSize(text, scale, encoding).x) : Mathf.RoundToInt((float)lineWidth / scale)) : 0);
		...
		while (num8 < length)
		{
			...
			else
			{
				...
				TUIBMGlyph glyph = mFont.GetGlyph(c);
				if (glyph != null)
				{
					if (num4 != 0)
					{
	...
	public void LoadBMFont()
	{
		if (!(null == fontTxt))
		{
			TUIBMFont.Reader.Load(mFont, base.name, fontTxt.bytes);
		}
	}
