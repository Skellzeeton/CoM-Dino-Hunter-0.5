		...
		public static void Load(TUIBMFont font, string name, byte[] bytes)
		{
			...
			while (tUIByteReader.canRead)
			{
				...
				if (array[0] == "char")
				{
					...
					int index = GetInt(array[1]);
					TUIBMGlyph glyph = font.GetGlyph(index, true);
					if (glyph != null)
					{
				...
				else if (array[0] == "kerning")
				{
					...
					int previousChar = GetInt(array[1]);
					int index2 = GetInt(array[2]);
					int amount = GetInt(array[3]);
					TUIBMGlyph glyph2 = font.GetGlyph(index2, true);
					if (glyph2 != null)
					{
						glyph2.SetKerning(previousChar, amount);
					}
				}
