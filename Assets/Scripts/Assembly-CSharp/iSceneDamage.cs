...
public class iSceneDamage : MonoBehaviour
{
	...
	{
		if (m_bActive && m_Mode != kMode.Continue)
		{
	...
	{
		if (m_bActive && m_Mode != kMode.Once)
		{
			CCharBase component = collider.transform.root.GetComponent<CCharBase>();
			if (!(component == null))
			{
	...
	protected void MakeDamage(CCharBase target)
	{
		...
		ResetTime(target);
		if (m_GameScene != null && m_GameScene.IsMyself(target))
		{
			target.OnHit(0f - m_fDamage, null, string.Empty);
			if (m_GameUI != null)
			{
				m_GameUI.AddDmgUI(m_fDamage, target.GetBone(1).position, Color.white, gyUILabelDmg.kMode.Mode2);
			}
			if (m_nBuffID > 0)
			{
