...
public class CCharPlayer : CCharBase
{
	...
	protected float m_fRecoverLifeTime = 10f;
	...
	public new void Update()
	{
		...
		if (value > 0f)
		{
			...
			if (m_fRecoverLifeTimeCount >= m_fRecoverLifeTime)
			{
				...
				AddHP(value);
				m_GameScene.AddHealText(value, GetBone(0).position);
			}
		}
	...
	public virtual void InitChar(int nCharID, int nLevel, int nExp = 0)
	{
		base.ID = nCharID;
		Level = nLevel;
		EXP = nExp;
		m_curCharacterInfo = m_GameData.GetCharacterInfo(nCharID);
	...
	public override void InitAnimData()
	{
		...
		case kCharMoveMode.Fly:
			...
			m_AnimData.Add(new CAnimInfo(kAnimEnum.Crossbow_Hurt, "crossbow_air_damage"));
			...
			m_AnimData.Add(new CAnimInfo(kAnimEnum.Crossbow_Skill, "crossbow_air_useskill"));
			m_AnimData.Add(new CAnimInfo(kAnimEnum.Crossbow_Rush, "crossbow_air_useskill_rush"));
			...
			m_AnimData.Add(new CAnimInfo(kAnimEnum.ShootGun_Hurt, "shootgun_air_damage"));
			...
			m_AnimData.Add(new CAnimInfo(kAnimEnum.ShootGun_Skill, "shootgun_air_useskill"));
			m_AnimData.Add(new CAnimInfo(kAnimEnum.ShootGun_Rush, "shootgun_air_useskill_rush"));
			...
			m_AnimData.Add(new CAnimInfo(kAnimEnum.AutoRifle_Hurt, "autorifle_air_damage"));
			...
			m_AnimData.Add(new CAnimInfo(kAnimEnum.AutoRifle_Skill, "autorifle_air_useskill"));
			m_AnimData.Add(new CAnimInfo(kAnimEnum.AutoRifle_Rush, "autorifle_air_useskill_rush"));
			...
			m_AnimData.Add(new CAnimInfo(kAnimEnum.HoldGun_Hurt, "holdgun_air_damage"));
			...
			m_AnimData.Add(new CAnimInfo(kAnimEnum.HoldGun_Skill, "holdgun_air_useskill"));
			m_AnimData.Add(new CAnimInfo(kAnimEnum.HoldGun_Rush, "holdgun_air_useskill_rush"));
			...
			m_AnimData.Add(new CAnimInfo(kAnimEnum.Rocket_Hurt, "rocket_air_damage"));
			...
			m_AnimData.Add(new CAnimInfo(kAnimEnum.Rocket_Skill, "rocket_air_useskill"));
			m_AnimData.Add(new CAnimInfo(kAnimEnum.Rocket_Rush, "rocket_air_useskill_rush"));
			...
			m_AnimData.Add(new CAnimInfo(kAnimEnum.Melee_Hurt, "melee_air_damage"));
			...
			m_AnimData.Add(new CAnimInfo(kAnimEnum.Melee_Skill, "melee_air_useskill"));
			m_AnimData.Add(new CAnimInfo(kAnimEnum.Melee_Rush, "melee_air_useskill_rush"));
	...
	{
		CWeaponInfoLevel weaponInfo = m_GameData.GetWeaponInfo(nWeaponID, nWeaponLevel);
		if (weaponInfo == null)
		{
			return;
		}
	...
	{
		if (movedir != kCharMoveDir.None && movedir != m_curMoveDir)
		{
	...
	{
		if (m_curAnim != kAnimEnum.Idle && (m_curAnim == kAnimEnum.MoveForward || m_curAnim == kAnimEnum.MoveBack || m_curAnim == kAnimEnum.MoveLeft || m_curAnim == kAnimEnum.MoveRight))
		{
			if (IsCanAttack())
			{
		...
		if (m_curMoveDir != kCharMoveDir.None)
		{
			SetDownBodyYaw(0f);
	...
	public override float CalcProtect(CWeaponInfoLevel weaponlevelinfo = null)
	{
		...
		float num = m_Property.GetValue(kProEnum.All_Protect);
		float num2 = 0f;
		switch (weaponlevelinfo.nAttackMode)
		{
		case 1:
	...
	{
		CItemInfo itemInfo = m_GameData.GetItemInfo(m_nCarryItemID);
		if (itemInfo != null)
		{
	...
	public virtual void UseSkill(int nSkill)
	{
		CSkillInfoLevel skillInfo = m_GameData.GetSkillInfo(nSkill, 1);
		if (skillInfo == null)
		{
			return;
		}
		...
		if (m_UseSkill != null)
		{
			if (skillInfo.nTargetLimit == 3)
			{
				...
				CGameNetSender.GetInstance().PlayerSkill(nSkill, base.UID);
			}
			else
			{
				m_UseSkill.Initialize(skillInfo);
				CGameNetSender.GetInstance().PlayerSkill(nSkill, -1);
			}
	...
	{
		Node behavior = m_GameData.GetBehavior(nBehaviorID);
		if (behavior != null)
		{
			if (m_Behavior == null)
			{
	...
	public void UseSkill(int nSkillID, int nTargetUID)
	{
		m_bNetSkill = true;
		m_nNetSkillID = nSkillID;
		m_pNetSkillTarget = m_GameScene.GetPlayer(nTargetUID);
	}
	...
	public virtual void Revive(float fHP)
	{
		...
		CrossAnim(kAnimEnum.Idle, WrapMode.Loop, 0.3f, 1f, 0f);
		ResetAI();
		GameObject gameObject = m_GameScene.AddEffect(base.Pos, Vector3.forward, 2f, 1300);
		if (gameObject != null)
		{
	...
	public void LevelUp(int nLevel)
	{
		...
		if (gameUI != null)
		{
			gameUI.SetProtraitLevel(nLevel, base.UID);
			gameUI.PlayProtraitLevelAnim(base.UID);
		}
		InitChar(base.ID, Level);
	}
}
