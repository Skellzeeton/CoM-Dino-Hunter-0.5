...
public class iWeaponCenter
{
	...
	public bool Load()
	{
		...
		{
			if (!Utils.FileGetString("weapon.xml", ref content))
			{
				return false;
			}
		}
		...
		{
			TextAsset textAsset = (TextAsset)Resources.Load(PrefabManager.GetPath(3004), typeof(TextAsset));
			if (textAsset == null)
			{
				return false;
			}
