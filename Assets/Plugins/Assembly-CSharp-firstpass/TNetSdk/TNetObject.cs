...
namespace TNetSdk
{
	public class TNetObject : TDispatchable
	{
		...
		private float last_received_msg_time;
		...
		protected void DisconnectCallback(IAsyncResult ar)
		{
			...
			{
				Event obj = new Event();
				obj.m_type = Event.TYPE.kDisconnect;
		...
		protected void SystemDisconnectCallback(IAsyncResult ar)
		{
			...
			{
				Event obj = new Event();
				obj.m_type = Event.TYPE.kSystemDisconnect;
				m_EventQueue.write(obj);
			}
			m_socket = null;
			m_Status = STATUS.kReady;
		...
		protected void SendDataEnd(IAsyncResult iar)
		{
			...
			Debug.Log("SendDataEnd " + Thread.CurrentThread.GetHashCode());
			try
			{
		...
		protected void Connected(IAsyncResult iar)
		{
			...
			catch (Exception)
			{
				...
				obj2.m_type = Event.TYPE.kConnectTimeout;
		...
		protected void RecvData(IAsyncResult iar)
		{
			...
			try
			{
				...
				if (num > 0)
				{
					...
					while (true)
					{
						...
						obj.m_packet = new Packet(m_RecvDataBuffer, num2, true);
						m_EventQueue.write(obj);
						m_iRcvLength -= num2;
					...
					{
						Event obj2 = new Event();
						obj2.m_type = Event.TYPE.kSystemClose;
						m_EventQueue.write(obj2);
				...
				else
				{
					...
					obj3.m_type = Event.TYPE.kSystemClose;
			...
			catch (Exception)
			{
				...
				obj4.m_type = Event.TYPE.kSystemClose;
		...
		public float GetTimeForNoRespone()
		{
			if (time_manager != null && time_manager.IsSynchronized())
			{
				return Time.time - last_received_msg_time;
			}
			return -1f;
		}
	}
